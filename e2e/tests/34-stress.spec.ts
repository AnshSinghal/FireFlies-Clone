...
test.describe("stress · a four-hour meeting @mutates", () => {
  ...
  test.afterAll(async ({ playwright }) => {
    /*
     * SOFT-deleted — `DELETE /meetings/{id}` sets `deleted_at` (T-28), it does
     * not remove the row. That is fine for the purpose: a soft-deleted meeting
     * drops out of every list, so the seed counts a dozen other specs assert on
     * are unaffected, and `globalSetup` rebuilds the database next run anyway.
     *
     * Said explicitly because an earlier version of this comment claimed the
     * opposite, and a comment that contradicts the endpoint it describes is
  ...
  test("T42-F · the DOM holds a window, not the whole transcript", async ({
    page,
  }) => {
    /*
    ...
    expect(
      rendered,
      `${rendered} rows in the DOM for ${SEGMENTS} segments`,
    ).toBeLessThan(120);
  });
  ...
  test("T42-F · find reaches a match 5,000 lines down", async ({ page }) => {
    ...
    await expect(page.getByText("zarquon clause").first()).toBeInViewport({
      timeout: 15_000,
    });
  });

  test("T42-F · clicking a line still seeks the player at this length", async ({
    page,
  }) => {
    ...
    await expect
      .poll(async () =>
        Number(await player.seekbar.getAttribute("aria-valuenow")),
      )
      .toBeGreaterThan(0);
  });
});
