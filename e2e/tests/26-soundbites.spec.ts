...
async function fetchClips(
  ...
  const response = await request.get(
    `${API_URL}/api/v1/meetings/${meetingId}/soundbites`,
  );
...
async function fetchSegments(
  ...
  const response = await request.get(
    `${API_URL}/api/v1/meetings/${meetingId}/transcript`,
  );
  expect(response.ok()).toBe(true);
...
async function selectAcross(
   ...
   * reports that as an eternal "element is outside of the viewport".
   *
   * `toBeVisible()` does not cover this: it means rendered, not on screen. The
...
test.describe("soundbites · clips, trimmer, range-locked playback", () => {
  test("T33-A · selecting two segments pre-fills the modal with their range", async ({
    ...
    expect(title).toContain(
      first.text.trim().split(/\s+/).slice(0, 3).join(" "),
    );
    expect(title.length).toBeGreaterThan(0);
    expect(title.length).toBeLessThanOrEqual(120);
  });

  test("T33-B · dragging the end handle updates the duration readout live", async ({
    page,
  }) => {
    await openMeeting(page);
    await openModal(page, 1, 2);
    ...
    expect(Number(await handle.getAttribute("aria-valuenow"))).toBeLessThan(
      endBefore,
    );
  });

  test("T33-C · a 1-second range is blocked with a min-length message", async ({
    page,
  }) => {
    await openMeeting(page);
    await openModal(page, 1, 2);
  ...
  test("T33-E · playing a soundbite seeks to its start and auto-pauses at its end", async ({
    ...
    const clip = clips.reduce((a, b) =>
      b.end_ms - b.start_ms < a.end_ms - a.start_ms ? b : a,
    );
    ...
    await expect
      .poll(() => position(page))
      .toBeGreaterThanOrEqual(startSec - 1);
  ...
  test("T33-G · seekbar bands span exactly each clip range", async ({
    page,
    request,
  }) => {
    const clips = await fetchClips(request, HERO);
    ...
    for (const clip of clips) {
      ...
      const expectedLeft =
        trackBox.x + trackBox.width * (clip.start_ms / durationMs);
      const expectedWidth =
        trackBox.width * ((clip.end_ms - clip.start_ms) / durationMs);
  ...
  test("T33-H · three Auto-badged proposals, each dismissible", async ({
    page,
  }) => {
    ...
    for (const index of [0, 1, 2]) {
      await expect(
        page.getByTestId(`soundbite-proposal-auto-${index}`),
      ).toContainText("Auto");
    }
  ...
  test("T33-K · a meeting with no soundbites shows the flyout empty state", async ({
    page,
  }) => {
    await openMeeting(page, BARE);
    await openFlyout(page);
...
test.describe("soundbites · save and delete", { tag: "@mutates" }, () => {
  ...
  test("T33-D · saving puts the card in the flyout, and it survives reload", async ({
    page,
  }) => {
    await openMeeting(page);
  ...
  test("T33-J · deleting removes the card and its seekbar band", async ({
    page,
  }) => {
    await openMeeting(page);
    await openFlyout(page);
