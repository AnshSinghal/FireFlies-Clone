...
import { useAiProvider, useAskFred } from '@/lib/api/ask'
...
export function AskFredPanel({ meetingId }: { meetingId: number }) {
  ...
  // From `/api/health` so the badge is right from panel OPEN; each answer's
  // own `provider` field then keeps it honest if they ever disagree.
  const { data: healthProvider } = useAiProvider()
  const [provider, setProvider] = useState<string | null>(null)
  const activeProvider = provider ?? healthProvider ?? null
